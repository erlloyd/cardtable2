...
- Update all YjsActions.ts functions to work with Y.Maps from `context.selection.yMaps`
- Update Board and BoardMessageBus to pass Y.Maps to renderer
- Update renderer message types to accept Y.Maps instead of plain objects
- Update migrations to work with Y.Maps directly
- **Update all tests** to use `yMaps` instead of `objects` in SelectionInfo mocks
- Search codebase for any remaining `.toJSON()` calls and eliminate them
- **Verify zero `.toJSON()` calls remain in the entire codebase**

**Key Patterns from T1-T4:**
```typescript
// Action handlers now receive Y.Maps
export function someAction(context: ActionContext) {
  const { yMaps } = context.selection; // Array of TableObjectYMap
...
// Type casts (when needed)
const yMap = new Y.Map() as TableObjectYMap;
yMap.set(key as keyof TableObjectProps, value as TableObjectProps[keyof TableObjectProps]);
```

**Common Changes Needed:**
- `context.selection.objects` → `context.selection.yMaps`
- `obj._kind` → `yMap.get('_kind')`
- `obj._pos` → `yMap.get('_pos')`
- `store.getAllObjects()` → `store.forEachObject()` or `store.objects`
- `store.getObject(id)` → `store.getObjectYMap(id)`
- ObjectChanges callbacks: `changes.added[].obj` → `changes.added[].yMap`
- ObjectChanges callbacks: `changes.updated[].obj` → `changes.updated[].yMap`

**Deliverables:**
- Refactored YjsActions.ts (all functions working with Y.Maps)
- Updated Board/BoardMessageBus to pass Y.Maps
- Updated renderer message types
- Updated migrations.ts
- Updated all test mocks to use `yMaps` field
- Grep verification: no `.toJSON()` calls in app/src (except debug toJSON() method)

**Test Plan:**
- All unit tests pass
- All E2E tests pass
- Renderer works correctly with Y.Maps
- Actions execute properly with Y.Maps
