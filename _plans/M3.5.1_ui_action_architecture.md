...
│                                                 │
│     ┌─────────────────────┐                    │
│     │ FloatingActionBar   │ ← appears on select│
│     │  [🔄] [↻] [⋮]      │                    │
│     └─────────────────────┘                    │
│              ↑                                  │
│        [Selected Card]                          │
...
### M3.5.1-T1: Action Registry System
...
### M3.5.1-T2: Keyboard Shortcut Manager
...
### M3.5.1-T3: Command Palette Component
...
### M3.5.1-T6: Floating Action Bar
**Objective:** Context-sensitive action bar above selection.

**Dependencies:** M3.5.1-T1, M3.5.1-T8 (Selection Context)

**Spec:**
- Position: 40px above selection center point
- Visibility:
  - Show when objects selected
  - Fade in (200ms) on selection
...
- Viewport boundary handling:
  - If no room above, flip to below selection (40px gap)
  - If no room on sides, center on screen
- Follow selection:
  - Re-calculate position when selected objects move
  - Smooth position updates (tween)
- "More" menu:
  - Dropdown with additional actions (ContextMenu component)
  - Opens downward from More button
- Integration:
  - Query `ActionRegistry.getAvailableActions()`
  - Filter to top 2-3 most relevant actions
  - Execute action on click

**Deliverables:**
- `app/src/components/FloatingActionBar.tsx`
- `app/src/components/FloatingActionBar.test.tsx`
- Position calculation utility: `app/src/utils/selectionBounds.ts`

**Test Plan:**
- E2E: Select object → bar appears above
- E2E: Deselect → bar fades out
- E2E: Bar auto-hides after 3s, returns on hover
- E2E: Bar follows selection when moved
- E2E: Layout adapts to single/multi/mixed selection
- Unit: Position calculation handles viewport boundaries
